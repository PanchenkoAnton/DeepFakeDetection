...
from models.xception import XceptionWSDAN
...
class BAP(nn.Module):
    ...
    def forward(self, features, attentions, dropout=False):
        ...
        feature_matrix = torch.sign(feature_matrix) * \
                         torch.sqrt(torch.abs(feature_matrix) + EPSILON)

        feature_matrix = F.normalize(feature_matrix, dim=-1)
        if dropout:
            feature_matrixd = torch.sign(feature_matrixd) * \
                              torch.sqrt(torch.abs(feature_matrixd) + EPSILON)
...
class WSDAN(nn.Module):
    def __init__(self, num_classes, M=32, net='inception_mixed_6e',
        ...
        self.fc = nn.Linear(self.M * self.num_features, self.num_classes,
                            bias=False)
        logging.info(
            'WSDAN based on Xception as feature extractor.\nnum_classes: {}, '
            'num_attentions: {}'.format(self.num_classes, self.M))
    ...
    def forward(self, x, dropout=False):
        ...
        if not dropout:
            ...
            feature_matrix, feature_matrixd = self.bap(feature_maps,
                                                       attention_maps,
                                                       dropout)
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        if len(pretrained_dict) == len(state_dict):
            logging.info('%s: All params were loaded.' % type(self).__name__)
        else:
