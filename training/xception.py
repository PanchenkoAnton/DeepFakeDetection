...
import torch
import numpy as np
import settings.xception as settings
...
class DFDCDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            result = self.transform(image=np.array(image))
            image = result["image"]
...
def _main():
    ...
    train_dataset = DFDCDataset(settings.data_list, "train", settings.data_root,
                                transform=train_transform)
    val_dataset = DFDCDataset(settings.data_list, "val", settings.data_root,
    ...
    if hasattr(settings, "resume") and os.path.isfile(settings.resume):
        ckpt = torch.load(settings.resume, map_location="cpu")
    ...
    for epoch in range(settings.n_epoches):
        ...
        with torch.no_grad():
            ...
            scheduler.step()
            ckpt_path = os.path.join(settings.save_dir, "ckpt-%d.pth" % epoch)
            save_checkpoint(
                ckpt_path,
                model.state_dict(),
                epoch=epoch + 1,
            ...
            if epoch_acc > best_acc:
                ...
                shutil.copy(ckpt_path,
                            os.path.join(settings.save_dir, "best.pth"))
                best_acc = epoch_acc
            print()
